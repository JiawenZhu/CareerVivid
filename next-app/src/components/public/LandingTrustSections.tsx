...
const userStories = [
    {
        ...
        avatarTone: 'bg-[#eef4ff] text-[#2563eb] border-[#dbe4f3]',
...
const PaperSection = ({ children, className = '' }: { children: ReactNode; className?: string }) => (
                ...
                <div className="flex items-center gap-3 rounded-xl border border-[#e4d3bc] bg-[#fffaf1]/88 px-4 py-3 shadow-sm">
                    <div className="flex -space-x-3" aria-hidden="true">
                        {userStories.map(({ role, avatar, avatarSrc, avatarTone }) => (
                            <span
                                key={role}
                                className={`flex h-11 w-11 items-center justify-center overflow-hidden rounded-full border-2 border-[#fffaf1] text-sm font-black shadow-sm ${avatarTone}`}
                            >
                                {avatarSrc ? (
                    ...
                    <div>
                        <p className="text-sm font-black text-[#211b16]">Example job-seeker stories</p>
                    ...
                    <article key={role} className="rounded-xl border border-[#e4d3bc] bg-[#fffaf1]/88 p-6 shadow-sm">
                        <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-3">
                                <div
                                    className={`flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full border text-sm font-black shadow-sm ${avatarTone}`}
                                >
                                    {avatarSrc ? (
                                        <img src={avatarSrc} alt="" className="h-full w-full object-cover" loading="lazy" />
                                    ) : (
                                ...
                                <div>
                                    <p className="text-xs font-black uppercase tracking-[0.14em] text-[#a97935]">Example story</p>
                                    <p className="text-sm font-black text-[#211b16]">{name}</p>
                                </div>
                            </div>
                            <CheckCircle2 size={20} className="shrink-0 text-[#15803d]" aria-hidden="true" />
                        </div>
                        <p className="mt-5 text-lg font-black leading-8 text-[#211b16]">“{quote}”</p>
                        <div className="mt-6 rounded-lg border border-[#eadbc5] bg-[#f9efe0]/80 p-4">
                            <p className="text-xs font-black uppercase tracking-[0.14em] text-[#a97935]">{role}</p>
    ...
    <PaperSection className="py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-12 grid gap-4 border-b border-[#e2d4c2] pb-10 sm:grid-cols-[1fr_auto] sm:items-end">
                ...
                <a
                    href="/job-tracker"
                    className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#d8c6ad] bg-[#fffaf1] px-5 py-3 text-sm font-black text-[#211b16] shadow-sm"
                >
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {workflowSteps.map(({ number, title, copy }) => (
                    <article key={title} className="rounded-xl border border-[#e4d3bc] bg-[#fffaf1]/88 p-6 shadow-sm">
                        <div className="mb-5 flex h-10 w-10 items-center justify-center rounded-full bg-[#211b16] font-mono text-sm font-black text-white">
                            {number}
                        </div>
                        <h3 className="text-xl font-black text-[#211b16]">{title}</h3>
                        <p className="mt-3 text-sm font-medium leading-6 text-[#665a4a]">{copy}</p>
                    </article>
                ))}
            </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-start lg:px-8">
            <div>
                ...
                <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                    ...
                    <a href="/contact" className="inline-flex items-center justify-center gap-3 rounded-lg border border-[#d8c6ad] bg-[#fffaf1] px-6 py-4 font-black text-[#211b16]">
                        Talk to us
                    </a>
                </div>
            </div>
