        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
                    <div>
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t('job_market.legend.title')}</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('job_market.legend.subtitle')}</p>
                    </div>
                ...
                <div className="p-6 space-y-4">
                    {HIGHLIGHT_LEGEND.map((item, idx) => (
                        <div key={idx} className="flex items-start gap-4">
                            ...
                            <div>
                                <div className="font-semibold text-gray-900 dark:text-white">{t(`job_market.legend.items.${item.id}.label`)}</div>
                                <div className="text-sm text-gray-500 dark:text-gray-400">{t(`job_market.legend.items.${item.id}.examples`)}</div>
                            </div>
                        </div>
                    ))}
                    <div className="flex items-start gap-4 pt-2 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <div>
                            <div className="font-semibold text-gray-900 dark:text-white">{t('job_market.legend.links.label')}</div>
                            <div className="text-sm text-gray-500 dark:text-gray-400">{t('job_market.legend.links.examples')}</div>
                        </div>
                    </div>
                </div>
            </div>
