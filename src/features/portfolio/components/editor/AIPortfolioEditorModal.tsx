...
const QUICK_PROMPT_DEFS = [
    { key: 'rewrite_about', icon: '✍️' },
    { key: 'punch_up_hero', icon: '⚡' },
    { key: 'add_project', icon: '🚀' },
    { key: 'add_skill', icon: '🛠️' },
    { key: 'clean_timeline', icon: '🗓️' },
    { key: 'remove_oldest_job', icon: '🗑️' },
];

const QUICK_STYLE_PROMPT_DEFS = [
    { key: 'gradient_headings', icon: '🎨' },
    { key: 'fade_in_sections', icon: '✨' },
    { key: 'left_accent_border', icon: '📐' },
    { key: 'rounded_buttons', icon: '🔵' },
    { key: 'card_shadows', icon: '🌗' },
    { key: 'tighter_typography', icon: '🔤' },
    { key: 'hover_effects', icon: '💫' },
    { key: 'reset_styles', icon: '🗑️' },
];
...
const AIPortfolioEditorModal: React.FC<AIPortfolioEditorModalProps> = ({
    ...
    const tAi = (key: string, options?: Record<string, unknown>) => t(`portfolio_editor.ai_modal.${key}`, options);
    ...
    const handleSend = async () => {
        ...
        setErrorMessage('');
        setProcessMessage(tAi('processing.reading_portfolio'));
        try {
            ...
            if (data.success && data.patch) {
                ...
                throw new Error(tAi('errors.no_changes'));
            }
        } catch (error: any) {
            setErrorMessage(error.message || tAi('errors.generic'));
        } finally {
    ...
    const handleGenerateFromResume = async () => {
        ...
        setIsProcessing(true);
        setErrorMessage('');
        setProcessMessage(tAi('processing.generating_from_resume'));
        try {
            const editFn = httpsCallable(functions, 'editPortfolio');
            ...
            if (data.success && data.patch) {
                ...
                throw new Error(tAi('errors.no_content'));
            }
        } catch (error: any) {
            setErrorMessage(error.message || tAi('errors.generic'));
        } finally {
    ...
    const handleGenerateCss = async () => {
        ...
        setErrorMessage('');
        setIsStyleApplied(false);
        setProcessMessage(tAi('processing.generating_styles'));
        try {
            ...
            setErrorMessage(error.message || tAi('errors.style_failed'));
        } finally {
            setIsProcessing(false);
        }
    };
    ...
    const sectionLabelFor = (section: string) => {
        const key = SECTION_KEYS[section];
        return key ? tAi(`sections.${key}`) : section;
    };
            ...
            `}>
                ...
                <header className={`flex-shrink-0 flex items-center justify-between px-6 py-4 border-b ${isDark ? 'border-white/10 bg-[#0f1117]' : 'border-gray-100 bg-white'}`}>
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div>
                            <h2 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{tAi('header.title')}</h2>
                            <p className={`text-xs flex items-center gap-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                                {tAi('header.powered_by')} <Zap size={10} className="text-yellow-400 fill-yellow-400" />
                            </p>
                        </div>
                        ...
                        {step === 'INPUT' && (
                            <div className={`ml-4 flex rounded-xl p-1 ${isDark ? 'bg-white/5' : 'bg-gray-100'}`}>
                                {([
                                    { id: 'edit', label: tAi('tabs.edit.label'), title: tAi('tabs.edit.title') },
                                    { id: 'generate', label: tAi('tabs.generate.label'), title: tAi('tabs.generate.title') },
                                    { id: 'style', label: tAi('tabs.style.label'), title: tAi('tabs.style.title') },
                    ...
                    {step === 'INPUT' && (
                        <div className={`w-full h-full flex flex-col p-6 md:p-8 ${isDark ? 'bg-[#0f1117]' : 'bg-white'}`}>
                            ...
                            {activeTab === 'edit' && (
                                <div className="w-full max-w-2xl mx-auto flex flex-col gap-6 h-full">
                                    <div className="text-center">
                                        <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>{tAi('edit.title')}</h3>
                                        <p className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                            {tAi('edit.description')}
                                        </p>
                                    </div>
                                    <div className="flex flex-wrap gap-2 justify-center">
                                        {QUICK_PROMPT_DEFS.map((qp) => (
                                            <button key={qp.key} onClick={() => setInstruction(tAi(`quick_prompts.${qp.key}.prompt`))}
                                                ...
                                                <span>{qp.icon}</span> {tAi(`quick_prompts.${qp.key}.label`)}
                                            </button>
                                        ))}
                                    </div>
                                    <div className="relative flex-1 flex flex-col">
                                        <textarea
                                            ...
                                            placeholder={tAi('edit.placeholder')}
                                            value={instruction}
                                            onChange={(e) => setInstruction(e.target.value)}
                                            onKeyDown={(e) => { if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) handleSend(); }}
                                            style={{ minHeight: '180px' }}
                                        />
                                        <div className={`absolute bottom-3 right-3 text-xs ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>{tAi('edit.send_shortcut')}</div>
                                    </div>
                                    ...
                                    <button onClick={handleSend} disabled={!instruction.trim() || isProcessing}
                                        ...
                                        {isProcessing ? <><Loader2 size={18} className="animate-spin" /> {processMessage}</> : <><Wand2 size={18} /> {tAi('edit.apply_with_ai')} <ChevronRight size={16} /></>}
                                    </button>
                                </div>
                            )}
                            ...
                            {activeTab === 'generate' && (
                                <div className="w-full max-w-2xl mx-auto flex flex-col gap-6 h-full">
                                    <div className="text-center">
                                        <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>{tAi('generate.title')}</h3>
                                        <p className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                    ...
                                    {resumes.length === 0 ? (
                                        <div className={`flex flex-col items-center justify-center gap-3 py-12 rounded-2xl border-2 border-dashed ${isDark ? 'border-white/10 text-gray-500' : 'border-gray-200 text-gray-400'}`}>
                                            ...
                                            <p className="text-sm font-medium">{tAi('generate.no_resumes_title')}</p>
                                            <p className="text-xs">{tAi('generate.no_resumes_description')}</p>
                                        </div>
                                    ) : (
                                        <div>
                                            <label className={`block text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{tAi('generate.select_resume')}</label>
                                            <div className="relative">
                                                <select value={selectedResumeId} onChange={(e) => setSelectedResumeId(e.target.value)}
                                                    ...
                                                    <option value="">{tAi('generate.choose_resume')}</option>
                                                    {resumes.map((r) => (
                                                        <option key={r.id} value={r.id}>
                                                            {r.title || tAi('generate.untitled_resume')}{r.personalDetails?.firstName ? ` (${r.personalDetails.firstName} ${r.personalDetails.lastName || ''})`.trim() : ''}
                                                        </option>
                                                    ))}
                                                </select>
                                            ...
                                            <div className={`rounded-xl border p-4 ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-100 shadow-sm'}`}>
                                                <div className="flex items-center gap-3 mb-3">
                                                    <div className="h-10 w-10 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-lg">
                                                        {(r.personalDetails?.firstName || r.title || tAi('generate.resume_initial'))[0].toUpperCase()}
                                                    </div>
                                                    <div>
                                                        <p className={`font-semibold text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>{r.title || tAi('generate.untitled_resume')}</p>
                                                        <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{r.personalDetails?.jobTitle || tAi('generate.resume_label')}</p>
                                                    </div>
                                                </div>
                                                <div className="flex flex-wrap gap-2">
                                                    ...
                                                    {r.skills && <span className={`text-xs px-2 py-1 rounded-full ${isDark ? 'bg-indigo-900/30 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>{tAi('generate.skills_count', { count: r.skills.length })}</span>}
                                                </div>
                                            </div>
                                        );
                                    })()}
                                    <div className={`rounded-xl p-4 border ${isDark ? 'bg-white/5 border-white/10' : 'bg-emerald-50 border-emerald-100'}`}>
                                        <p className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? 'text-gray-400' : 'text-emerald-700'}`}>{tAi('generate.populated_title')}</p>
                                    ...
                                    <button onClick={handleGenerateFromResume} disabled={!selectedResumeId || isProcessing}
                                        ...
                                        {isProcessing ? <><Loader2 size={18} className="animate-spin" /> {processMessage}</> : <><Sparkles size={18} /> {tAi('generate.button')} <ChevronRight size={16} /></>}
                                    </button>
                                </div>
                            )}
                            ...
                            {activeTab === 'style' && (
                                <div className="w-full max-w-2xl mx-auto flex flex-col gap-5 h-full overflow-y-auto">
                                    <div className="text-center">
                                        <h3 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>{tAi('style.title')}</h3>
                                        <p className={`text-sm mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                            {tAi('style.description')}
                                        </p>
                                    </div>

                                    {/* Quick Style Chips */}
                                    <div>
                                        <p className={`text-xs font-semibold uppercase tracking-wider mb-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{tAi('style.quick_styles')}</p>
                                        <div className="flex flex-wrap gap-2">
                                            {QUICK_STYLE_PROMPT_DEFS.map((q) => (
                                                <button key={q.key} onClick={() => setStyleInstruction(tAi(`quick_style_prompts.${q.key}.prompt`))}
                                                    ...
                                                    <span>{q.icon}</span> {tAi(`quick_style_prompts.${q.key}.label`)}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="relative">
                                        <textarea
                                            ...
                                            rows={3}
                                            placeholder={tAi('style.placeholder')}
                                    ...
                                    <button onClick={handleGenerateCss} disabled={!styleInstruction.trim() || isProcessing}
                                        ...
                                        {isProcessing ? <><Loader2 size={16} className="animate-spin" /> {processMessage}</> : <><Code2 size={16} /> {tAi('style.generate_css')} <ChevronRight size={14} /></>}
                                    </button>
                                            ...
                                            <div className={`flex items-center justify-between px-4 py-2.5 ${isDark ? 'bg-white/5' : 'bg-gray-50'} border-b ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-xs font-semibold ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                                                        {isStyleApplied ? tAi('style.applied') : (aiStyleSummary || tAi('style.generated_css'))}
                                                    </span>
                                                </div>
                                                <button onClick={() => navigator.clipboard.writeText(generatedCss || '')}
                                                    className={`p-1.5 rounded-lg transition-colors ${isDark ? 'hover:bg-white/10 text-gray-400' : 'hover:bg-gray-200 text-gray-500'}`} title={tAi('style.copy')}>
                                                    <Copy size={14} />
                                                </button>
                                            </div>
                                            ...
                                            {!isStyleApplied && (
                                                <div className={`flex gap-2 p-3 ${isDark ? 'bg-white/5' : 'bg-gray-50'} border-t ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
                                                    <button onClick={() => handleApplyStyle('replace')}
                                                        ...
                                                        <Check size={14} /> {tAi('style.replace_apply')}
                                                    </button>
                                                    {currentCss && (
                                                        <button onClick={() => handleApplyStyle('append')}
                                                            ...
                                                            <Code2 size={14} /> {tAi('style.append')}
                                                        </button>
                                                    )}
                                                </div>
                                            )}
                                        ...
                                        <div className={`rounded-xl p-4 border ${isDark ? 'bg-white/5 border-white/10' : 'bg-amber-50 border-amber-100'}`}>
                                            <div className="flex items-center justify-between mb-2">
                                                <span className={`text-xs font-semibold ${isDark ? 'text-gray-400' : 'text-amber-700'}`}>
                                                    {tAi('style.active_css', { count: currentCss.length })}
                                                </span>
                                                <button onClick={handleResetCss} className="flex items-center gap-1 text-xs text-red-500 hover:text-red-600 transition-colors">
                                                    <Trash2 size={12} /> {tAi('style.remove_all')}
                                                </button>
                                            </div>
                            ...
                            <div className={`w-80 flex-shrink-0 flex flex-col h-full overflow-y-auto ${isDark ? 'bg-[#0a0c12]' : 'bg-gray-50'}`}>
                                <div className="p-5 space-y-5">
                                    <div className={`rounded-xl border p-4 ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-100 shadow-sm'}`}>
                                        <div className="flex items-center gap-2 mb-3">
                                            ...
                                            <p className={`text-xs font-bold uppercase tracking-wider ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                                                {activeTab === 'generate' ? tAi('preview.portfolio_generated') : tAi('preview.ai_applied_changes')}
                                            </p>
                                        </div>
                                    ...
                                    {changedSections.length > 0 && (
                                        <div>
                                            <p className={`text-xs font-bold uppercase tracking-wider mb-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                                                {activeTab === 'generate' ? tAi('preview.sections_generated') : tAi('preview.sections_modified')}
                                            </p>
                                            <div className="flex flex-wrap gap-2">
                                    ...
                                    {activeTab === 'edit' && (
                                        <div>
                                            <p className={`text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-1.5 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                                                <MessageSquare size={12} /> {tAi('preview.try_again')}
                                            </p>
                                            <div className="relative">
                                                <textarea
                                                    ...
                                                    placeholder={tAi('preview.try_again_placeholder')}
                                                    value={instruction}
                                                    onChange={(e) => setInstruction(e.target.value)}
                                                    rows={3}
                                                />
                                                <button onClick={handleSend} disabled={!instruction.trim() || isProcessing}
                                                    className="absolute bottom-2 right-2 p-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors" title={tAi('preview.rerun')}>
                                    ...
                                    <button onClick={() => setStep('INPUT')}
                                        ...
                                        <RotateCcw size={13} /> {tAi('preview.start_over')}
                                    </button>
                                </div>
                            </div>
                    ...
                    {isProcessing && (
                        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
                            <div className={`p-8 rounded-2xl shadow-2xl flex flex-col items-center max-w-xs text-center border ${isDark ? 'bg-[#0f1117] border-white/10' : 'bg-white border-gray-100'}`}>
                                ...
                                <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{tAi('loading.subtitle')}</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                {step === 'PREVIEW' && (
                    <footer className={`flex-shrink-0 flex items-center justify-between px-6 py-4 border-t ${isDark ? 'border-white/10 bg-[#0a0c12]' : 'border-gray-100 bg-white'}`}>
                        <p className={`text-xs italic ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>{tAi('footer.unsaved_note')}</p>
                        <div className="flex gap-3">
                            ...
                            <button onClick={handleApply} className="px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-lg shadow-green-600/20 transition-all flex items-center gap-2">
                                <Check size={16} /> {tAi('footer.apply_changes')}
                            </button>
                        </div>
                    </footer>
                )}
