...
const getActiveGoals = (jobApplications: JobApplicationData[], targetRoles: string[]): string[] => {
    const nextActions = compactUnique(
        jobApplications
            .map((job) => job.nextAction)
            .filter((action) => action && action !== 'No action'),
        4
    );

    if (nextActions.length > 0) return nextActions;
    if (targetRoles.length > 0) return targetRoles.map((role) => `Grow fit for ${role}`).slice(0, 3);
    return [];
};
...
export const buildCareerProfileGraph = ({
    ...
    const activeGoals = getActiveGoals(jobApplications, targetRoles);
    ...
    const nodes: CareerProfileGraphNode[] = [
        ...
        },
        {
            id: 'goals',
            label: 'Goals',
            value: activeGoals.length ? pluralize(activeGoals.length, 'goal') : 'No goals',
            detail: activeGoals[0] || 'Track a target role or next action to clarify direction.',
            progress: clamp((activeGoals.length / 3) * 100),
            tags: activeGoals.slice(0, 3),
