...
import { PlusCircle, Trash2, Wand2, UploadCloud, User, Briefcase, GraduationCap, Link as LinkIcon, Star, CheckCircle, Loader2, Brush, Languages, Zap, ChevronDown, ChevronUp, GripVertical, PenTool } from 'lucide-react';
...
const ResumeForm: React.FC<ResumeFormProps> = ({ resume, onChange, tempPhoto, setTempPhoto, isReadOnly = false }) => {
    ...
    // Helper for section title inputs
    const SectionTitleInput = ({ fieldKey, label }: { fieldKey: keyof ResumeData['sectionTitles'], label: string }) => (
        <div className="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <Input 
                id={`sectionTitles.${fieldKey as string}`}
                label={`Custom Title for "${label}"`}
                value={resume.sectionTitles?.[fieldKey] || label} 
                onChange={e => handleChange(fieldKey, e.target.value, 'sectionTitles')}
                placeholder={label}
                disabled={isReadOnly}
            />
        </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <FormSection title="Professional Summary" icon={<Briefcase className="text-primary-500" />}>
                <SectionTitleInput fieldKey="summary" label="Professional Summary" />
                <EditableTextarea 
            ...
            <FormSection title="Websites & Social Links" icon={<LinkIcon className="text-primary-500" />}>
                {resume.websites.map((link, index) => (
                 ...
                 <button onClick={() => addArrayItem('websites', { id: crypto.randomUUID(), label: 'Website', url: '', icon: 'link' })} disabled={isReadOnly} className="flex items-center gap-2 text-sm text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 disabled:opacity-50 disabled:cursor-not-allowed"><PlusCircle size={16}/> Add Link</button>
            </FormSection>

            <FormSection title="Skills" icon={<Star className="text-primary-500" />}>
                 <SectionTitleInput fieldKey="skills" label="Skills" />
            ...
            <FormSection title="Languages" icon={<Languages className="text-primary-500" />}>
                 <SectionTitleInput fieldKey="languages" label="Languages" />
            ...
            <FormSection title="Employment History" icon={<Briefcase className="text-primary-500" />}>
                 <SectionTitleInput fieldKey="experience" label="Employment History" />
            ...
            <FormSection title="Education" icon={<GraduationCap className="text-primary-500" />}>
                 <SectionTitleInput fieldKey="education" label="Education" />
