...
export const ModernTemplate: React.FC<TemplateProps> = ({ resume, themeColor, titleFont, bodyFont, onUpdate, onFocus }) => {
  ...
  // Helper for headers to use InlineEdit mapped to sectionTitles
  const SectionHeader = ({ titleKey, defaultTitle }: { titleKey: keyof typeof sectionTitles, defaultTitle: string }) => (
      <h3 className="text-sm font-bold uppercase text-gray-500 tracking-widest mb-3" style={titleStyle}>
          <InlineEdit 
              value={sectionTitles[titleKey] || defaultTitle} 
              fieldId={`sectionTitles.${String(titleKey)}`}
              onFocus={onFocus}
    ...
    <div className="p-10 text-gray-800 bg-white" style={bodyStyle}>
      <header className="text-left mb-8">
        ...
        <div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 mt-4">
            ...
            {websites.map((site, index) => (
                <a key={site.id} href={site.url} target="_blank" rel="noopener noreferrer" className="hover:text-primary-600 flex items-center gap-1" style={{ color: themeColor }}>
      ...
      <main>
        <section className="mb-6">
          <SectionHeader titleKey="summary" defaultTitle="Profile" />
          <InlineEdit 
        ...
        <section className="mb-6">
          <SectionHeader titleKey="skills" defaultTitle="Skills" />
        ...
        <section className="mb-6">
          <SectionHeader titleKey="experience" defaultTitle="Experience" />
        ...
        <section>
          <SectionHeader titleKey="education" defaultTitle="Education" />
