                            ...
                            {name === 'Use Cases' && (
                                <>
                                    ...
                                    <a href="/bio-links" className={`flex items-start gap-3 p-3 transition-colors group/item ${isBrutalist ? 'hover:bg-pink-200 border-2 border-transparent hover:border-black' : 'rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                                        ...
                                        <div>
                                            <div className={`text-sm ${isBrutalist ? 'font-black text-black uppercase' : 'font-semibold text-gray-900 dark:text-white'}`}>{t('nav.bio_links') || "Link in Bio"}</div>
                                            <p className={`text-xs ${isBrutalist ? 'text-black font-bold' : 'text-gray-500 dark:text-gray-400'}`}>{t('nav.bio_links_desc') || "Share everything"}</p>
                                        </div>
                                    </a>
                                </>
                            )}
                            {/* ... Rest of dropdowns ... */}
                            {/* Keeping simplified for brevity in edit, assuming other dropdowns follow pattern or hidden in this specific snippet replacement */}
                            {name === 'Resources' && (
                                <>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    ...
                    <nav className="hidden md:flex items-center gap-8 h-full">
                        {context !== 'bio-link' && <NavItem name={t('nav.product')} href="/" />}
                        <NavItem name={t('nav.use_cases')} hasDropdown />
                        <NavItem name="Partners" href="/partners" />
                        <NavItem name={t('nav.pricing')} href={isBrutalist ? "#pricing" : "/pricing"} />
                ...
                <div className={`md:hidden absolute top-20 left-0 right-0 z-40 ${isBrutalist
                    ...
                    <div className="px-4 py-6 flex flex-col gap-4">
                        ...
                        <div className="space-y-1">
                            {context !== 'bio-link' && (
                                <a href="/" onClick={() => setIsMenuOpen(false)} className={`block py-2 ${isBrutalist ? 'text-lg font-black uppercase text-black hover:bg-yellow-200 px-2 -mx-2' : 'text-base font-semibold text-gray-900 dark:text-white'}`}>{t('nav.product')}</a>
                            )}
                            ...
                            <div className={`pl-4 ${isBrutalist ? 'border-l-4 border-black ml-1 my-2 space-y-2' : 'border-l-2 border-gray-100 dark:border-gray-800 space-y-3 my-2'}`}>
                                ...
                                <a href="/bio-links" onClick={() => setIsMenuOpen(false)} className={`block ${isBrutalist ? 'font-bold text-black uppercase hover:underline' : 'text-sm text-gray-600 dark:text-gray-400'}`}>{t('nav.bio_links') || "Link in Bio"}</a>
                            </div>

                            <a href={isBrutalist ? "#pricing" : "/pricing"} onClick={() => setIsMenuOpen(false)} className={`block py-2 ${isBrutalist ? 'text-lg font-black uppercase text-black hover:bg-green-200 px-2 -mx-2' : 'text-base font-semibold text-gray-900 dark:text-white'}`}>{t('nav.pricing')}</a>
