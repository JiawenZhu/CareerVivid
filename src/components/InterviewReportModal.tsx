...
const InterviewReportModal: React.FC<InterviewReportModalProps> = ({ jobHistoryEntry, onClose, isGuestMode = false }) => {
    ...
    const formatAnalysisToText = (analysis: InterviewAnalysis) => {
        ...
        const transcriptSource = analysis.transcript?.length ? analysis.transcript : (jobHistoryEntry.transcript || []);
        transcriptSource.forEach(t => {
                    ...
                    <div ref={printRef} className="bg-white" style={{ width: '800px', padding: '40px' }}>
                        ...
                        <div style={{ pageBreakBefore: 'always', marginTop: '40px' }}>
                            ...
                            <div className="space-y-4">
                                {(currentAnalysis.transcript?.length ? currentAnalysis.transcript : (jobHistoryEntry.transcript || [])).map((entry, index) => (
                    ...
                    {currentAnalysis ? (
                        <>
                            ...
                            <footer className="p-4 border-t dark:border-gray-700 flex justify-end gap-3 flex-shrink-0">
                                {isGuestMode ? (
                                    <a href="/signin" className="bg-primary-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-primary-700">
