...
const Sidebar: React.FC = () => {
    ...
    const { currentUser, userProfile, logOut, aiUsage, isPremium } = useAuth();
    ...
    const handleGlobalSave = useCallback(async () => {
        ...
        try {
            ...
            if (currentNodesStr !== lastSavedNodesRef.current) {
                ...
                // Sidebar node updates are stored safely in profile if needed
            }
        } catch (e) {
            console.error(e);
        }
    ...
    // Filter dynamic database assets (resumes, portfolios, whiteboards, posts)
    const activeDocuments = nodes.filter(n =>
        n.data?.type !== 'project' &&
        n.data?.type !== 'system'
    );
    ...
    return (
        <aside className="fixed inset-y-0 left-0 w-64 bg-white/60 dark:bg-gray-900/40 backdrop-blur-2xl border-r border-white/20 dark:border-gray-800/40 shadow-[4px_0_24px_rgba(0,0,0,0.02)] dark:shadow-[4px_0_24px_rgba(0,0,0,0.2)] hidden md:flex flex-col z-30 transition-transform duration-300">
            ...
            <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-4 select-none">
                ...
                {/* NAVIGATION SECTION */}
                <div>
                    <div className="flex items-center justify-between px-2 group mb-2 text-[10px] font-extrabold uppercase tracking-widest text-gray-400 dark:text-gray-500">
                    ...
                    <div className="space-y-1.5">
                        {activeDocuments.map(doc => (
                            <div 
                                ...
                                className={`group/doc flex items-center gap-2 px-2.5 py-2 rounded-xl hover:bg-white/40 dark:hover:bg-gray-800/20 border border-transparent hover:border-gray-200/40 dark:hover:border-gray-800/40 cursor-pointer text-xs transition-all ${activeNodeId === doc.id.toString() ? 'bg-indigo-50/50 dark:bg-indigo-500/10 font-bold border-l-2 border-indigo-500 text-indigo-600 dark:text-indigo-400' : 'font-semibold text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100'}`}
                            >
                                ...
                                <button
                                    ...
                                    className="opacity-0 group-hover/doc:opacity-100 p-0.5 hover:bg-gray-100 dark:hover:bg-gray-850 rounded text-gray-400 transition-opacity"
                                >
            ...
            <div className="mt-auto flex flex-col gap-1 px-4 pt-4 pb-2 border-t border-transparent relative before:absolute before:inset-x-4 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-gray-200 dark:before:via-gray-700 before:to-transparent shrink-0">
                ...
                <div className="grid grid-cols-2 gap-1 mb-2.5">
                    ...
                    <button 
                        ...
                        className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-[11px] font-bold transition-all border ${currentPath === '/interview-studio' ? 'bg-indigo-500 text-white border-indigo-600 shadow-md shadow-indigo-500/10' : 'text-gray-655 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800/60 border-transparent'}`}
                    >
                ...
                {currentUser ? (
                    <button onClick={logOut} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg font-medium text-gray-655 hover:bg-red-50 hover:text-red-600 transition-colors text-sm">
