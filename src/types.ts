...
// --- Auto-Apply Agent Types ---
export type EeoAnswer = 'prefer_not_to_answer' | 'not_provided' | string;
export type SalaryPreferenceType = 'annual' | 'hourly';
export type ApplicationWorkModelPreference = 'remote' | 'hybrid' | 'onsite' | 'flexible';
...
export interface ApplicationProfile {
  ...
  eeo: {
    gender: EeoAnswer;
    raceEthnicity: EeoAnswer;
    disabilityStatus: EeoAnswer;
    veteranStatus: EeoAnswer;
  };
  compensation: {
    targetSalary: string;
    minimumSalary: string;
    preferenceType: SalaryPreferenceType;
  };
  relocationRemote: {
    willingToRelocate: boolean | null;
    preferredLocations: string[];
    workModelPreference: ApplicationWorkModelPreference;
  };
  availability: {
    startDate: string;
    workSchedule: string;
...
export interface ApplicationAnswerPlanItem {
  label: string;
  answer: string;
...
export interface ApplicationAnswerPlan {
  answers: ApplicationAnswerPlanItem[];
  missingRequiredLabels: string[];
  sensitiveBlockedLabels: string[];
  generatedAt: any;
}

export interface ApplicationRunReceiptStep {
  id?: string;
  type: 'field' | 'file' | 'navigation' | 'submit' | 'screenshot' | 'error' | 'needs_user';
  label?: string;
  value?: string;
  status: 'filled' | 'skipped' | 'uploaded' | 'visited' | 'submitted' | 'failed' | 'needs_user';
  message?: string;
...
export interface ApplicationRunReceipt {
  ...
  jobId: string;
  userId: string;
  executor: 'extension' | 'desktop' | 'manual';
...
export interface ApplicationQueueItem {
  ...
  riskFlags: string[];
  answerPlan?: ApplicationAnswerPlan;
