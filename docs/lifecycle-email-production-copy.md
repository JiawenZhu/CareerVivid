...
## Measurable Activation Loops

These lifecycle emails should be treated as product loops, not static broadcasts. Each trigger must come from a real user event, every optional send must pass profile preference and frequency gates, and every queued email records `usage_logs.eventType = hydrated_lifecycle_email_queued` plus `users/{uid}.lifecycleEmails.sent.{key}`.

| Loop | Trigger event | Email key | Goal | Primary CTA | Success metric |
| --- | --- | --- | --- | --- | --- |
| Welcome -> first resume | No resume after signup delay | `onboarding_welcome` | `first_resume_created` | Signup/profile start or dashboard | User creates first resume |
| First resume -> role-specific application | First resume exists without match analysis | `first_resume_completed_tailor_job` | `first_resume_tailor_started` | Active resume editor | User runs match analysis or opens tailoring flow |
| Review complete -> score suggestions | `resume_review_completed` or `resume_match_analysis` | `review_completed_score_suggestions` | `review_suggestions_opened` | Active resume editor | User returns to editor and applies/acts on suggestions |
| Job saved -> application packet | First `users/{uid}/jobTracker/{jobId}` create | `first_job_saved_application_packet` | `saved_job_application_packet_started` | Job tracker / active resume editor | User generates cover letter, tailors resume, or advances job status |
| Shared resume viewed -> recruiter engagement | Public resume API logs `shared_resume_viewed` | `shared_resume_recruiter_engagement` | `shared_resume_recruiter_engagement_opened` | Public shared resume or editor | User reviews shared resume and recruiter-readiness state |
| Interview practice reminder | `interview_analysis` | `interview_practice_reminder` | `interview_practice_returned` | Interview studio | User starts another practice or opens interview report |
| Clear value -> advocacy | `resume_download`, `shared_resume_viewed`, or `interview_analysis` | `advocacy_value_request` | `advocacy_feedback_started` | Profile/feedback path | User leaves feedback, review, or referral signal |

Implementation files:

- Hydrated templates and CTAs: `functions/src/emailDataBinding.ts`
- Trigger orchestration and lifecycle event timestamps: `functions/src/lifecycleEmails.ts`
- Resume review measurement: `src/contexts/AIReviewContext.tsx`
- Public recruiter/shared-resume measurement: `functions/src/publicResumeApi.ts`
