...
- Lifecycle email foundation:
  ...
  - Activation-loop emails now map to measurable product events: welcome -> first resume, first resume -> tailor one role, review complete -> score suggestions, first job saved -> application packet, shared resume viewed -> recruiter engagement, interview analysis -> practice reminder, and clear value -> advocacy request.
