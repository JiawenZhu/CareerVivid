...
exports.getFinancialMetrics = exports.applyDiscount = exports.cancelSubscription = exports.stripeWebhook = exports.createCheckoutSession = void 0;
 ...
 */
exports.createCheckoutSession = (0, https_2.onCall)({
    ...
    try {
        ...
        try {
            ...
            if (error.code === 'resource_missing' && error.param === 'customer') {
                ...
                await admin.firestore().collection("users").doc(userId).set({ stripeCustomerId: newCustomerId }, { merge: true });
        ...
        throw new https_2.HttpsError("internal", `Failed to create checkout session: ${error.message}`);
    }
});
/**
 ...
 */
exports.cancelSubscription = (0, https_2.onCall)({
    secrets: [stripeSecretKey],
    region: "us-west1",
}, async (request) => {
    if (!request.auth) {
    ...
    try {
        ...
        if (!stripeSubscriptionId) {
            ...
            throw new https_2.HttpsError("not-found", "No active subscription found to cancel.");
        }
        ...
        await sendCancellationConfirmationEmail(userData?.email, userData?.displayName || "there", cancelDate);
        return {
            status: "success",
        ...
        throw new https_2.HttpsError("internal", `Failed to cancel subscription: ${error.message}`);
    }
});
/**
 * Applies a retention discount to an active subscription
 */
exports.applyDiscount = (0, https_2.onCall)({
    secrets: [stripeSecretKey],
    region: "us-west1",
}, async (request) => {
    if (!request.auth) {
    ...
    if (!['RETENTION_10', 'RETENTION_20'].includes(discountType)) {
        throw new https_2.HttpsError("invalid-argument", "Invalid discount type");
    }
    const userId = request.auth.uid;
    const stripe = new stripe_1.default(stripeSecretKey.value(), {
        apiVersion: "2025-11-17.clover",
    ...
    try {
        ...
        if (!stripeSubscriptionId) {
            throw new https_2.HttpsError("not-found", "No active subscription found to apply discount.");
        }
        ...
        throw new https_2.HttpsError("internal", `Failed to apply discount: ${error.message}`);
    }
});
/**
...
async function handleInvoicePaid(invoice) {
    ...
    const userData = userDoc.data();
    // Send invoice email
    await sendInvoiceEmail(userData.email, userData.displayName || "Valued Customer", invoice);
...
async function handleInvoicePaymentFailed(invoice) {
    ...
    await sendPaymentFailedEmail(userData.email, userData.displayName || "Valued Customer", invoice);
    console.log(`Payment failed email sent to user ${userDoc.id}`);
}
/**
 * Send invoice email using Firestore mail collection
 */
async function sendInvoiceEmail(userEmail, userName, invoice) {
 ...
 */
async function sendPaymentFailedEmail(userEmail, userName, invoice) {
    const mailRef = admin.firestore().collection("mail").doc();
    const invoiceNumber = invoice.number || invoice.id;
 ...
 * Send cancellation confirmation email with win-back CTA
 */
async function sendCancellationConfirmationEmail(userEmail, userName, accessEndDate) {
    const mailRef = admin.firestore().collection("mail").doc();
 ...
 */
function generateInvoiceEmailHTML(userName, invoiceNumber, amount, date, planName, invoiceUrl) {
    return `
<!DOCTYPE html>
<html>
<head>
 ...
 */
function generateInvoiceEmailText(userName, invoiceNumber, amount, date, planName, invoiceUrl) {
    return `
Payment Received
 ...
 * Generate HTML email for payment failed
 */
function generatePaymentFailedEmailHTML(userName, invoiceNumber, amount, invoiceUrl) {
    const APP_URL = "https://careervivid.app";
    return (0, emailTemplates_1.generateNeoBrutalistEmail)({
 ...
 */
function generatePaymentFailedEmailText(userName, invoiceNumber, amount, invoiceUrl) {
    return `
 ...
 */
exports.getFinancialMetrics = (0, https_2.onCall)({
    secrets: [stripeSecretKey],
    region: "us-west1",
}, async (request) => {
    // Verify authentication
    ...
    try {
        ...
        throw new https_2.HttpsError("internal", `Failed to fetch financial metrics: ${error.message}`);
    }
});
