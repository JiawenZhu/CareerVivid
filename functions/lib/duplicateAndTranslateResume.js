...
exports.duplicateAndTranslateResume = void 0;
const functions = __importStar(require("firebase-functions"));
const admin = __importStar(require("firebase-admin"));
const translate_1 = require("@google-cloud/translate");
// --- Duplicate and Translate Resume Function ---
exports.duplicateAndTranslateResume = functions.region('us-west1').https.onCall(async (data, context) => {
    ...
    if (!resumeId || !targetLanguage) {
        throw new functions.https.HttpsError('invalid-argument', 'The function must be called with "resumeId" and "targetLanguage" arguments.');
    }
    ...
    try {
        ...
        if (currentCount >= limit) {
            throw new functions.https.HttpsError('resource-exhausted', `Translation limit reached (${limit}/month). Please upgrade or contact support for a limit increase.`);
        }
        ...
        // Employment History
        resumeCopy.employmentHistory?.forEach((job, i) => {
        ...
        // Education
        resumeCopy.education?.forEach((edu, i) => {
        ...
        // Skills
        resumeCopy.skills?.forEach((skill, i) => {
        ...
        if (translations.length !== textsToTranslate.length) {
            throw new functions.https.HttpsError('internal', 'Translation mismatch: expected and received counts differ.');
        }
        // 7. Rehydrate - Map translations back to resume copy
        fieldMap.forEach(({ path, index }) => {
            const translatedText = translations[index];
            const pathParts = path.split('.');
            let current = resumeCopy;
        ...
        throw new functions.https.HttpsError('internal', 'Failed to duplicate and translate resume.', error.message);
    }
});
//# sourceMappingURL=duplicateAndTranslateResume.js.map